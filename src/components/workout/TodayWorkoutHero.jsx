...
export default function TodayWorkoutHero({ day, exercises, activeSession, completedSession }) {
  ...
  if (!day || day.isRest) {
    ...
    return (
      <section className="limit-hero mt-5 rounded-[2rem] p-6">
        <p className="text-[10px] font-bold uppercase tracking-[.2em] text-muted-foreground">Today</p>
        <h2 className="mt-2 text-3xl font-extrabold tracking-tight">Recovery day</h2>
        <p className="mt-1 text-sm text-muted-foreground">{next ? `Next: ${next.name} · ${next.label}` : 'Rest and refuel.'}</p>
      </section>
    );
  }
  const working = exercises.reduce((a, x) => a + (x.sets || 3), 0);
  const minutes = estimateMinutes(exercises);
  const progression = exercises.find(item => item.progressionNotes)?.progressionNotes;
  if (completedSession) {
    ...
    return (
      <section className="limit-hero mt-5 rounded-[2rem] border-primary/35 p-6 shadow-[0_20px_60px_hsl(var(--primary)/.1)]">
        <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.2em] text-primary"><CheckCircle2 className="h-3.5 w-3.5" /> Completed today</p>
        <h2 className="mt-2 text-3xl font-extrabold tracking-tight">{day.name}</h2>
  ...
  if (activeSession) {
    ...
    return (
      <section className="limit-hero mt-5 rounded-[2rem] border-primary/50 p-6 shadow-[0_20px_60px_hsl(var(--primary)/.14)]">
        <p className="text-[10px] font-bold uppercase tracking-[.2em] text-primary">Workout in progress</p>
        <h2 className="mt-2 text-3xl font-extrabold tracking-tight">{activeSession.name}</h2>
  ...
  return (
    <section className="limit-hero mt-5 rounded-[2rem] p-6">
      ...
      {exercises.length > 0 && <p className="mt-3 text-xs font-bold tabular-nums text-muted-foreground">{exercises.length} exercises · {working} working sets · ~{minutes} min</p>}
      {progression&&<p className="mt-3 border-l-2 border-primary pl-3 text-xs font-semibold text-foreground">{progression}</p>}
