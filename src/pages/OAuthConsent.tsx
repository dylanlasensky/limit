...
export default function OAuthConsent() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const data: ConsentInfo = await res.json();
  ...
  const respond = async (action: "approve" | "deny") => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          const returnTo = window.location.pathname + "?ctx=" + encodeURIComponent(ctx as string);
          const encoded = encodeURIComponent(returnTo);
          window.location.href =
  ...
  const tools: ConsentTool[] = Array.isArray(info?.tools) ? info!.tools! : [];
