...
export default function SetRow({ row, previous, onEdit, onToggle, saving }) {
  ...
  return (
    <div className="mt-2 grid grid-cols-[20px_54px_minmax(0,1fr)_minmax(0,1fr)_36px_40px] items-center gap-2">
      ...
      <span className="text-center text-xs tabular-nums text-muted-foreground">{previous ? `${previous.weight}×${previous.reps}` : '—'}</span>
      ...
      <motion.button whileTap={{ scale: .88 }} disabled={saving} onClick={()=>{if(!done)navigator.vibrate?.(12);onToggle()}} aria-label={done ? 'Uncheck set' : 'Complete set'}
