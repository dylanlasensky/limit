...
export const goalKey = (g?: string | null): GoalKey => {
  g = (g || "").toLowerCase();
  ...
  const base =
    experience === "beginner"
      ? role === "iso"
        ? 2
        : 3
      : experience === "advanced" && (role === "main" || role === "main_strength")
        ? 4
        : 3;
  if (role === "main_strength")
    return { sets: experience === "beginner" ? 3 : 4, repMin: 3, repMax: 6, rest: 210 };
  if (goal === "strength")
    return role === "main"
...
const S = (muscles: string[], role: string, type?: string): Slot => ({ muscles, role, type });
const FOCUS: Record<string, Slot[]> = {
  full: [
    S(["Quads"], "main", "Compound"),
    S(["Chest"], "main", "Compound"),
    S(["Lats", "Upper back"], "main"),
    S(["Hamstrings", "Glutes"], "secondary", "Compound"),
    S(["Front delts", "Side delts"], "secondary"),
    S(["Biceps", "Triceps"], "iso", "Isolation"),
    S(["Abs/core"], "iso"),
  ],
  upper: [
    S(["Chest"], "main", "Compound"),
    S(["Upper back", "Lats"], "main"),
    S(["Front delts"], "secondary"),
    S(["Lats", "Upper back"], "secondary"),
    S(["Side delts"], "iso", "Isolation"),
    S(["Triceps"], "iso", "Isolation"),
    S(["Biceps"], "iso", "Isolation"),
  ],
  lower: [
    S(["Quads"], "main", "Compound"),
    S(["Hamstrings"], "main", "Compound"),
    S(["Glutes"], "secondary", "Compound"),
    S(["Quads"], "secondary"),
    S(["Hamstrings"], "iso", "Isolation"),
    S(["Calves"], "iso", "Isolation"),
    S(["Abs/core"], "iso"),
  ],
  push: [
    S(["Chest"], "main", "Compound"),
  ...
  squat: [
    S(["Quads"], "main_strength", "Compound"),
    S(["Quads"], "secondary"),
    S(["Hamstrings"], "secondary", "Compound"),
    S(["Calves"], "iso"),
    S(["Abs/core"], "iso"),
  ],
  bench: [
    S(["Chest"], "main_strength", "Compound"),
    S(["Chest"], "secondary"),
    S(["Front delts"], "secondary"),
    S(["Triceps"], "iso", "Isolation"),
    S(["Side delts"], "iso"),
  ...
  deadlift: [
    S(["Hamstrings"], "main_strength", "Compound"),
    S(["Lats"], "main"),
    S(["Upper back"], "secondary"),
    S(["Glutes"], "secondary", "Compound"),
    S(["Biceps"], "iso"),
  ],
  upper_assist: [
...
export const slotsFor = (dayName: string, profile: any): Slot[] => {
  ...
  if (
    dayName.toLowerCase().includes("strength") &&
    focus !== "squat" &&
    focus !== "bench" &&
    focus !== "deadlift"
  )
    slots = slots.map((s, i) => (i === 0 ? { ...s, role: "main_strength" } : s));
  (profile.priorityMuscles || []).forEach((p: string) => {
    ...
    if (
      covering.length &&
      covering.length < 3 &&
      ["upper", "lower", "full", "push", "pull", "arms_shoulders"].includes(focus)
    )
      slots.push(S(muscles, "iso", "Isolation"));
  });
  return slots;
};
...
export const targetMusclesFor = (dayName: string): string[] =>
  (
    ({
      ...
      upper_assist: ["Shoulders", "Back", "Arms"],
...
const whyFor = (key: string, { goal, exp, days, prios }: WhyContext): string => {
  const goalText: string = {
    muscle: "build muscle",
    strength: "get stronger",
    muscle_strength: "build muscle and strength together",
    fat_loss: "lose fat while keeping muscle",
    general: "improve overall fitness",
  }[goal];
  const prioText = prios?.length
    ? ` Your priority areas (${prios.join(", ")}) get extra direct work.`
    : "";
  return (
    {
...
export const scoreProgramStructures = (
  ...
  const goal = goalKey(profile.fitnessGoal),
    ...
    prios: string[] = profile.priorityMuscles || [];
  const specLabel =
    prios.includes("Shoulders") || prios.includes("Arms")
      ? "Shoulders + Arms"
      : prios.some((p) => ["Glutes", "Quads", "Hamstrings", "Calves"].includes(p))
        ? "Lower Focus"
        : "Upper Focus";
  const c: ProgramCandidate[] = [];
  if (days <= 4)
    c.push({
      key: "full_body",
      name: "Full Body",
      dayNames: ["Full Body A", "Full Body B", "Full Body C", "Full Body D"].slice(0, days),
      score:
        58 +
        (exp === "beginner" ? 24 : exp === "intermediate" ? 4 : -6) +
        (days <= 3 ? 14 : -4) +
        (len <= 45 ? 8 : 0) +
        (goal === "fat_loss" || goal === "general" ? 10 : 0),
    });
  if (days === 4)
    c.push({
      key: "upper_lower",
      ...
      score:
        ...
        14 +
        (exp !== "beginner" ? 10 : 0) +
        (goal === "muscle" || goal === "muscle_strength" ? 8 : 0),
    });
  ...
  if (days === 6)
    c.push({
      ...
      score:
        64 +
        (exp === "advanced" ? 14 : exp === "intermediate" ? 8 : -20) +
        (goal === "muscle" ? 10 : 0),
    });
  if (days === 6)
    c.push({
      key: "upper_lower",
      name: "Upper / Lower ×3",
      dayNames: ["Upper A", "Lower A", "Upper B", "Lower B", "Upper C", "Lower C"],
      score: 60 + (exp === "beginner" ? 10 : 0),
    });
  if (days === 5)
    c.push({
      key: "upper_lower_spec",
      name: `Upper / Lower + ${specLabel}`,
      dayNames: ["Upper A", "Lower A", specLabel, "Upper B", "Lower B"],
      score:
        66 + (prios.length ? 14 : 4) + (exp !== "beginner" ? 8 : 0) + (goal === "muscle" ? 8 : 0),
    });
  if (days === 4 && goal === "muscle_strength")
    c.push({
      key: "powerbuilding",
      name: "Powerbuilding",
      dayNames: ["Upper Strength", "Lower Strength", "Upper Hypertrophy", "Lower Hypertrophy"],
      score: 84 + (exp !== "beginner" ? 6 : -16),
    });
  if (goal === "strength" && days >= 3 && days <= 4)
    c.push({
      key: "strength_focus",
      name: "Strength Focus",
      dayNames:
      ...
      score: 82 + (exp === "beginner" ? -14 : 6),
    });
  const options = c
    .filter((x) => x.dayNames.length === days)
