...
export default async function (req) {
  try {
    ...
    const result = await withWorkoutLock(client, user, async (assertLock, profile) => {
      ...
      if (input.action === "activatePlan") return activatePlan(db, user, input.planId, assertLock);
      ...
      if (input.action === "saveSet") {
        ...
        if (
          ...
          !profile.equipment.some((e) => /full|commercial|gym/i.test(e)) &&
          exercise.equipment !== "Bodyweight" &&
          !profile.equipment.some((e) => e.toLowerCase().includes(exercise.equipment.toLowerCase()))
        )
      ...
      if (session.status !== "completed") {
        ...
        const volume = Math.round(sets.reduce((a, r) => a + +r.weight * +r.reps, 0)),
        ...
        const summary = {
          ...
          completedExercises: new Set(sets.map((s) => s.workoutExerciseId || s.exerciseName)).size,
          volume,
          prs,
          ratingChanges: [],
          analyticsPending: true,
          bestLift: best ? { name: best.exerciseName, weight: best.weight, reps: best.reps } : null,
