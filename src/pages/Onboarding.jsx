...
export default function Onboarding() {
  ...
  return (
    <main className={`${dark ? 'dark' : ''} mx-auto min-h-screen max-w-md overflow-hidden bg-background px-5 pb-8 pt-[max(2rem,env(safe-area-inset-top))] text-foreground`}>
      <div className="flex items-center justify-between"><b className="text-xl tracking-[.24em]">LIMIT</b><span className="text-xs font-bold tabular-nums text-muted-foreground">{step+1} / {steps.length}</span></div>
      ...
      <div className="my-7">
        ...
        <h1 className="mt-2 text-4xl font-black leading-[1.05] tracking-tight">{title}</h1><p className="mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">Your answers shape your training frequency, exercise selection, session length, and starting nutrition targets.</p>
      </div>
      ...
      <button disabled={saving || !canContinue} onClick={() => step < steps.length - 1 ? setStep(step + 1) : finish()}
        className={`mt-8 h-14 w-full rounded-2xl font-bold text-white transition-all ${canContinue&&!saving?'bg-blue-600 shadow-lg shadow-blue-950/30':'bg-zinc-800 text-zinc-500'}`}>
