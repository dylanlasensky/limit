        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {allergens.map((x) => (
            <button
              ...
              className={`rounded-full border px-3 py-2 text-sm ${(data.allergies || []).includes(x) ? "border-red-500 bg-red-50 text-red-700" : ""}`}
            >
              {x}
            </button>
          ))}
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {prefs.map((x) => (
            <button
              ...
              className={`rounded-full border px-3 py-2 text-sm ${(data.dietaryPreferences || []).includes(x) ? "border-blue-500 bg-blue-50 text-blue-800" : ""}`}
            >
              {x}
            </button>
          ))}
