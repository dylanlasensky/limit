  ...
  return (
    <div className="space-y-3">
      {goals.map(([label, value, hint]) => (
        <button key={value} type="button" onClick={() => { set('fitnessGoal', value); set('importAfterOnboarding', false); }}
      ...
      <div className="pt-3"><p className="limit-kicker mb-2">Already have a plan?</p><button type="button" onClick={()=>{set('fitnessGoal','follow existing program');set('importAfterOnboarding',true)}} className={`flex min-h-16 w-full items-center gap-3 rounded-2xl border px-4 text-left ${data.importAfterOnboarding?'border-primary/60 bg-primary/10':'border-border/70 bg-secondary/40'}`}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-background"><Upload className="h-4 w-4 text-primary"/></span><span><b className="block text-sm">Use my own program</b><span className="text-xs text-muted-foreground">Import it after setup</span></span>{data.importAfterOnboarding&&<Check className="ml-auto h-5 w-5 text-primary"/>}</button></div>
    </div>
  );
}