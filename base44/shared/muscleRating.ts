...
const bodyWeightLb = (profile: Record<string, any> | undefined, weights: any[]): number => {
  ...
  const value = +profile?.currentWeight;
  if (!Number.isFinite(value) || value <= 0) return 170;
  return profile?.measurementSystemVersion === "us_v1" || profile?.units === "imperial"
    ? value
    : value * 2.20462;
};
export const calculateMuscleRating = ({
  ...
  const exerciseByName: Record<string, any> = Object.fromEntries(
    exercises.filter((x) => typeof x.name === "string").map((x) => [x.name.toLowerCase(), x])
  );
  ...
  const allSessions = new Set(eligibleSets.map((x) => x.workoutSessionId).filter(Boolean)).size;
